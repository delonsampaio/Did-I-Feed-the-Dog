...
struct HelpView: View {
    ...
    private var feedingStatusSection: some View {
        Section("Feeding Status") {
            ...
            FAQRow(
                ...
                answer: "Yes. Go to Settings -> Dogs -> [dog's name] -> Medical -> Medications and tap Add Medication. Enter the name, optional dose, and how often it needs to be given (twice daily, daily, every 2 days, every 3 days, or weekly). When a dose is due, a purple 💊 banner appears on your dog's card — tap it to log the dose. Pro users can also enable a push notification for each medication so the app reminds you when the next dose is due."
            )
        }
    }
    ...
    private static let allFAQs: [FAQItem] = [
            ...
            .init(section: "Widgets & Quick Actions", question: "How do I assign the Action Button to log a feeding?", answer: "On supported iPhones, go to iPhone Settings -> Action Button -> Shortcuts, then choose \"Log Feeding\" from the Did I Feed the Dog? shortcuts list. Press the Action Button from anywhere — even when the screen is off — to instantly log a meal."),
